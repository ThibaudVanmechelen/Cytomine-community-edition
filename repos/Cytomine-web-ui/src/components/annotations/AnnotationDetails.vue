...
export default {
  ...
  methods: {
    ...
    async sendSamRequest() {
      const startTime = performance.now(); // DELETE
      try {
        const annotationId = this.annotation.id;
        const response = await Cytomine.instance.api.post(`annotation/${annotationId}/sam`);

        const endTime = performance.now(); // End timing DELETE
        const duration = (endTime - startTime).toFixed(2); // Duration in milliseconds DELETE

        console.log(`SAM request took ${duration} ms`); // DELETE
